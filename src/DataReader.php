...
class DataReader implements \Countable, \ArrayAccess, \Iterator {
	 ...
	 * Constructor
	 * 
	 * @param Response $response The command generating the query result.
	 * 
	 * @return void
	 * @since XXX
	 */
	public function __construct(Response $response = null) {
		...
		if ($response !== null) {
			if ($response->isMultipart === true) {
				...
				$this->_objects[] = $this->buildObject($response);
			}
	 ...
	 * </code>
	 * 
	 * @param Response $response  build the object from response.
	 * @param mixed    $objectKey The key of object to build.
	 * 
	 * @return array
	 * @since  XXX
	 */
		...
		if ($response->isMultiPart === true) {
			...
			foreach ($ret[self::HEADERS_KEY] as $key => $value) {
				...
				if (preg_match('/X-Riak-([^-]+)-?(.*)/i', $key, $matches) > 0) {
					...
					if ($type == 'Index') {
						$data = explode('_', $data);
						$ret[self::INDEX_KEY][lcfirst($data[0])] = array($value, $data[1]);
					} else {
						if ($type == 'Vclock') {
	...
	public function current() {
		if (isset($this->_objects[$this->_index]) === true) {
	...
	public function offsetGet($offset) {
		return (isset($this->_objects[$offset]) === true ? $this->_objects[$offset] : null);		
	}
	 ...
	 * 
	 * @param Response $response  The response to build.
	 * @param string   $objectKey The object key of the object to add.
	 * 
	 * @return void
	 * @since  XXX
	 */
	public function addObject(Response $response, $objectKey = null) {
		$this->_objects[] = $this->buildObject($response, $objectKey);
	}
