...
class Query extends Component {
	 ...
	 */
	protected $_basicQuorum;

	/**
	 * @var string The notfound_ok query parameters
	 */
	protected $_notFounfOk;
	...
	protected $_returnTerms;
	
	protected $_streaming;
...
//		$this->_mode = 'select';
	 ...
	 *
	 * @param Connection $noSqlDb the database connection used to generate the query statement.
	 ...
	 *
	 * @param Connection $db the database connection used to generate the query statement.
	 * If this parameter is not given, the `db` application component will be used.
	 * 
	 ...
	 *
	 * @param mixed $bucketName is name of bucket or array of inputs for map/reduce/link query
	 * 
	 ...
	 * 
	 * @param mixed $objectKey is the key of object (string | int)
	 * 
	 * @return Query (set the key of object)
	 * @since  XXX
	 */
	public function withKey($objectKey) {
		if (isset($this->_mode) === true) {
	...
	public function withMapReduce($inputs = null) {
		...
		$this->_mode = 'selectWithMapReduce';
		...
		if (is_string($inputs)) {
			...
			throw new InvalidParamException('Inputs should be a string reprensenting a bucket or an array of bucket, key, keydata. (array($bucket, $key, $keydata))');
		}
		return $this;
	}
	 ...
	 * 
	 * @param Map $map The map phase to add to the mapReduce
	 * 
	 * @return Query The query object itself
	 * @since  XXX
	 */
	 ...
	 * 
	 * @param Reduce $reduce The reduce phase to add to the mapReduce.
	 * 
	 * @return Query The query object itself
	 * @since  XXX.
	 */
	public function reduce(Reduce $reduce) {
		$this->_mapReduce->addPhase($reduce);
		return $this;
	}
	 ...
	 * 
	 * @param Link $link The link phase to add to the mapReduce.
	 * 
	 * @return Query The query object itself.
	 * @since  XXX
	 */
	public function link(Link $link) {
		$this->_mapReduce->addPhase($link);
		return $this;
	}
	 ...
	 * Search from index
	 *
	 * @param string $indexName The name of searched index
	 * @param string $value     The value of the searched index (Or the startValue if $endValue is setted)
	 * @param string $endValue  The end value of the searched index.
	 ...
	 * Search link.
	 *
	 * @param string $bucketName the bucketname (set '_' to search in any bucket)
	 * @param string $riakTag    The searched tag (set '_' to match any tag)
	 * @param string $keep       The keep argument ('0' or '1' or '_')
	 * 
	 * <code>
	 ...
	 * <code>
	 * 		$query->select()->fromBucket('user')->withKey('userTest')->linked('user', 'friend', '0')->addLinked('user', 'friend', '1');
	...
	public function addLinked($bucketName = '_', $riakTag = '_', $keep = '_') {
		$this->_links[] = array(
			implode(',', array($bucketName, $riakTag, $keep))
		);
		return $this;
	}
	...
	/**
	 * Set the number of objects wanted. (only for search withIndex)
	 * 
	 * @param integer $limit The maximum number of wanted object. 
	 * 
	 * @return Query The query object itself
	 * @since  XXX
	 */
	...
	/**
	 * Set the return_terms ('true' or 'false'). (only for search withIndex)
	 * 
	 * @param string $returnTerms The return_terms.
	 * 
	 * @return Query The query object itself.
	 */
	public function returnTerms($returnTerms = 'true') {
		$this->_returnTerms = $returnTerms;
		return $this;
	}
	 ...
	 * Set the r value.
	 * 
	 * @param int $value How many replicas need to agree when retrieving the object (if not specified, the bucket r value will be used).
	 * 
	 * @return Query The query object itself.
	 * @since  XXX
	 */
	 ...
	 * Set the pr value
	 * 
	 * @param integer $value How many primary replicas need to be online when doing the read  (if not specified, the bucket r value will be used).
	 * 
	 * @return Query The query object itself
	 * @since  XXX
	 */
	 ...
	 * Set the notFoundOk
	 * 
	 * @param string $value Whether to treat notfounds as successful reads for the purposes of R 
	 ...
	 * Set the vtag.
	 * 
	 * @param string $value When accessing an object with siblings, which sibling to retrieve. 
	 * 
