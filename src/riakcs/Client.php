...
class Client extends Component
{
     ...
     *
     * @param boolean $useSslValidation
     *            use ssl validation ?
     *
     * @return void
     * @since XXX
    ...
    {
        $this->useExceptions = $useException;
    }
     ...
     *
     * @param string $bucket
     *            The bucket name wanted to get
     * @param array $parameters
     *            An array of key => value which represents paramName => paramValue
     * @param array $headers
     *            An array of key => value which represents headerName => headerValue.
     *
     * @return array false array or false, if object not found.
     * @since XXX
     */
        ...
        try {
            ...
            if ($response->getStatus() === 200) {
                ...
                foreach ($contents as $content) {
                    ...
                    $ownerId = $content->getElementsByTagName('Owner')
                        ->item(0)
                        ->getElementsByTagName('ID')
                        ->item(0)->textContent;
                    $displayName = $content->getElementsByTagName('Owner')
                        ->item(0)
     ...
     *            The bucket name wanted to get
     * @param string $acl
     *            The access control list to set to the bucket.
     * @param array $headers
     *            An array of key => value which represents headerName => headerValue.
     *
     * @return boolean
     ...
     *
     * @param string $bucket
     *            The bucket name wanted to delete
     *
     * @return boolean
     * @since XXX
     */
     ...
     *
     * @param string $bucket
     *            The bucket which contains $objectKey.
     * @param string $objectKey
     *            The object key wanted to get.
     *
     * @return array false
     * @since XXX
     */
     ...
     *            The object key wanted to get.
     * @param mixed $data
     *            The object data to insert in BDD
     *            (if is array, automatically json_encode + contentType = 'application/json')
     * @param string $contentType
     *            The type of data to insert.
     * @param string $acl
     *            The acl for the object.
     * @param array $metaHeaders
     *            Additionnal meta to attach to the object.
     * @param array $headers
     *            Additionnal headers
     *
     ...
     *            The bucket which contains $objectKey.
     * @param string $objectKey
     *            The object key wanted to get info.
     *
     * @return array boolean information array about the object, or false if an error occured.
     * @since XXX
     */
     ...
     *            The bucket which contains $objectKey.
     * @param string $objectKey
     *            The object key wanted to delete.
     *
     ...
     *
     * @param string $bucket
     *            The bucket name where you want to upload the object.
     * @param string $objectKey
     *            The key of the object wanted to upload.
     * @param string $filename
     *            The file wanted to upload.
     * @param array $metaData
     *            Metadata to store with the object.
     * @param string $acl
     *            The Access Control to set for the file.
     * @param integer $partSize
     *            The size of each part you want to upload (in MB).
     *
     ...
     *
     * @param string $bucket
     *            The bucket name where the object has to be stored.
     * @param string $objectKey
     *            The key of object to be stored.
     * @param string $filename
     *            The file to upload.
     * @param array $metaData
     *            The metaData to attach to the object.
     * @param string $acl
     *            The access control list.
     * @param integer $partSize
     *            The size of each part to upload. (if partsize > objectSize, it will upload it in one part).
     *
    ...
    public function initMultiPartUpload($bucket, $objectKey, $filename, $acl = self::ACL_PRIVATE, $metaData = array())
    {
        ...
        $request = $this->createRequest('POST', $bucket, $objectKey, $headers, array(
            'uploads' => ''
        ));
        $response = $request->execute();
     ...
     *            The uploadId generates by Client::initMultiPartUpload().
     * @param integer $partSize
     *            The size of each part wanted to upload.
     *
     ...
     *            The key of the object wanted to upload.
     * @param array $parts
     *            All parts (partNumber => Etag value) generates by uploadAllPart.
     * @param string $uploadId
     *            The current uploadId (generates by initMultiPartUpload).
     *
     ...
     *
     * @param array $parts
     *            An array which contains partNumber => ETag of the part (This array is generated by uploadAllParts)
     *
     ...
     *            The bucket name.
     *
     * @return array false of active uploads or false if an error occurred.
     * @since XXX
     */
     ...
     *            The objcet key.
     * @param array $headers
     *            The request headers (pair key => value).
     * @param array $parameters
     *            The request parameters (pair key => value).
     *
    ...
    private function createRequest($verb, $bucket, $objectKey = '', $headers = array(), $parameters = array())
    {
        ...
        $contentMd5 = isset($headers['Content-MD5']) === true &&
                      strlen($headers['Content-MD5']) > 0 ?
                          $headers['Content-MD5'] :
                          '';
        $contentType = (isset($headers['Content-Type']) === true &&
                       strlen($headers['Content-Type']) > 0) ?
    ...
    private function buildAcl($data)
    {
        ...
        $ownerId = $doc->getElementsByTagName('Owner')
            ->item(0)
            ->getElementsByTagName('ID')
            ->item(0)->textContent;
        $ownerName = $doc->getElementsByTagName('Owner')
            ->item(0)
            ->getElementsByTagName('DisplayName')
            ->item(0)->textContent;
        $ret['ownerId'] = $ownerId;
        ...
        foreach ($grants as $grant) {
            $userId = $grant->getElementsByTagName('Grantee')
                ->item(0)
                ->getElementsByTagName('ID')
                ->item(0)->textContent;
            $username = $grant->getElementsByTagName('Grantee')
                ->item(0)
                ->getElementsByTagName('DisplayName')
                ->item(0)->textContent;
